...
private:
    ...
    template<typename U> friend struct SimpleSerializer;
public:
    struct Node {
        T value;
    ...
    void insert(const std::vector<std::size_t>& path, const T& v) {
        if (path.empty()) {
            if (root_) {
                throw std::logic_error("root exists");
            }
        ...
        if (last >= max_children_ || cur->children[last]) {
            throw std::logic_error("invalid insert");
        }
    ...
    bool containsSubtree(const Node* pat) const {
        bool ok = false;
        preorder(root_, [&](Node* n){ if (!ok && equalsSubtree(n, pat)) ok = true; });
        return ok;
    }
